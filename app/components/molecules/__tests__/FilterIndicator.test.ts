...
describe("FilterIndicator molecule", () => {
  it("renders label, filter value badge, and clear button", () => {
    ...
    expect(
      indicator.classList.contains("workout-filter-indicator"),
    ).toBe(true);

    // Check label text
    const labelEl = indicator.querySelector(
      ".workout-filter-indicator-text",
    );
    expect(labelEl?.textContent).toBe("Filtering by: ");

    // Check badge
    const badge = indicator.querySelector(
      ".workout-filter-indicator-badge",
    );
    expect(badge?.textContent).toBe("Drop Set");

    // Check clear button
    const clearBtn = indicator.querySelector(
      ".workout-filter-indicator-clear",
    );
  ...
  it("uses custom className when provided", () => {
    ...
    expect(
      indicator.classList.contains(
        "workout-protocol-filter-indicator",
      ),
    ).toBe(true);
    expect(
      indicator.classList.contains("workout-filter-indicator"),
