...
describe("ListItem molecule", () => {
  describe("createList", () => {
    ...
    it("creates ul with custom class", () => {
      ...
      expect(
        list.classList.contains("workout-muscle-group-list"),
      ).toBe(true);
    });
  ...
  describe("create", () => {
    it("creates li with label and value", () => {
      ...
      expect(
        item.classList.contains("workout-muscle-group-item"),
      ).toBe(true);
      expect(
    ...
    it("renders icon when provided", () => {
      ...
      expect(
        item.querySelector(".workout-list-item-icon")?.textContent,
      ).toBe("🏋️");
    });
    ...
    it("renders secondary text when provided", () => {
      ...
      expect(
        item.querySelector(".workout-list-item-secondary")
          ?.textContent,
      ).toBe("2024-01-15");
    });
    ...
    it("uses custom label and value classes", () => {
      ...
      expect(item.querySelector(".custom-label")?.textContent).toBe(
        "Test",
      );
      expect(item.querySelector(".custom-value")?.textContent).toBe(
        "100",
      );
    });
  });
  ...
  describe("createSimple", () => {
    it("creates simple label-value item", () => {
      ...
      expect(item.tagName).toBe("LI");
      expect(item.classList.contains("workout-exercise-item")).toBe(
        true,
      );
  ...
  describe("createText", () => {
    it("creates text-only list item", () => {
      ...
      expect(item.classList.contains("workout-mapping-item")).toBe(
        true,
      );
    });
    ...
    it("applies custom value class", () => {
      ...
      expect(
        item
          ...
          ?.classList.contains("highlight-value"),
      ).toBe(true);
    });
  });
  ...
  describe("createEmpty", () => {
    it("creates empty list item for custom content", () => {
      ...
      expect(item.tagName).toBe("LI");
      expect(item.classList.contains("workout-custom-item")).toBe(
        true,
      );
