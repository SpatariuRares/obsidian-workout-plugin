...
describe("ChartLegendItem", () => {
  it("renders color indicator and text", () => {
    ...
    const colorBox = item.querySelector(
      ".chart-legend-color",
    ) as HTMLElement;
    ...
    expect(
      item.querySelector(".chart-legend-label")?.textContent,
    ).toBe(BODY_PARTS.UPPER_BODY);
    expect(
      item.querySelector(".chart-legend-value")?.textContent,
    ).toBe("45%");
  });
  ...
  describe("interactivity", () => {
    it("adds clickable class and calls onClick when clicked", () => {
      ...
      expect(
        item.classList.contains("chart-legend-item-clickable"),
      ).toBe(true);
    ...
    it("adds active class when isActive is true", () => {
      ...
      expect(
        item.classList.contains("chart-legend-item-active"),
      ).toBe(true);
    });
    ...
    it("adds dimmed class when isDimmed is true", () => {
      ...
      expect(
        item.classList.contains("chart-legend-item-dimmed"),
      ).toBe(true);
