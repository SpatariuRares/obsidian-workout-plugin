...
describe("WorkoutEventBus", () => {
  ...
  describe("on / emit", () => {
    it("should call handler when matching event is emitted", () => {
      ...
      bus.emit({
        type: "log:added",
        payload: { entry, context: { exercise: "Squat" } },
      });
    ...
    it("should coalesce log events into log:bulk-changed", async () => {
      ...
      await bus.batch("import", async () => {
        ...
        bus.emit({
          type: "log:added",
          payload: {
            entry: makeEntry("Bench"),
            context: { exercise: "Bench" },
          },
        });
        bus.emit({
          type: "log:added",
          payload: {
            entry: makeEntry("Deadlift"),
            context: { exercise: "Deadlift" },
          },
        });
      });
  ...
  describe("error handling", () => {
    ...
    it("should wrap non-Error thrown values in an Error object", () => {
      ...
      expect(errorHandler).toHaveBeenCalledTimes(1);
      expect(errorHandler.mock.calls[0][0].error).toBeInstanceOf(
        Error,
      );
      expect(errorHandler.mock.calls[0][0].error.message).toBe(
        "string error",
...
describe("normalizeExercise", () => {
  it.each([
    ...
    const {
      normalizeExercise,
    } = require("@app/services/events/WorkoutEventTypes");
