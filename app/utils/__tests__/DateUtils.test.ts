...
describe("DateUtils", () => {
  describe("extractDateOnly", () => {
    ...
    it("should handle ISO strings with timezone", () => {
      const result = DateUtils.extractDateOnly(
  ...
  describe("formatDate", () => {
    ...
    it("should handle different times on same day", () => {
      ...
      expect(DateUtils.formatDate(date1)).toBe(
        DateUtils.formatDate(date2),
      );
    });
  });
  ...
  describe("getDaysAgo", () => {
    it("should return date N days ago", () => {
      ...
      expect(DateUtils.formatDate(result)).toBe(
        DateUtils.formatDate(expected),
      );
    });
  ...
  describe("filterByTimeFrame", () => {
    ...
    const mockData: WorkoutLogData[] = [
      ...
      {
        date: DateUtils.formatDate(
          new Date(today.getTime() - 400 * 24 * 60 * 60 * 1000),
        ),
        ...
        workout: "Old Workout",
        timestamp: new Date(
          today.getTime() - 400 * 24 * 60 * 60 * 1000,
        ).getTime(),
      },
    ];
  ...
  describe("formatDateWithFormat", () => {
    ...
    it("should format date with DD/MM/YYYY explicitly", () => {
      const result = DateUtils.formatDateWithFormat(
        "2024-01-15",
        "DD/MM/YYYY",
      );
      expect(result).toBe("15/01/2024");
