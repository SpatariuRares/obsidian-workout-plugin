...
const STRING_FILTERS = [
  ...
  // CSS colors
  (s) =>
    /^(white|black|red|green|blue|yellow|orange|purple|gray|grey|transparent|inherit|currentColor|none|unset|initial)$/i.test(
      s,
    ) ||
  ...
  (s) =>
    /^(top|bottom|left|right|center|middle|start|end|flex|grid|block|inline|none|auto|hidden|visible|absolute|relative|fixed|sticky|row|column|wrap|nowrap|stretch|baseline|space-between|space-around|space-evenly|vertical|horizontal)$/.test(
      s,
    ),
  ...
  (s) =>
    /^\.[a-z]{1,5}$/.test(s) ||
    /\.(csv|json|md|txt|yaml|yml|xml|html|css|js|ts|tsx|py|sh|canvas|png|jpg|svg|mp3|wav|ogg)$/i.test(
      s,
    ),

  // HTML element tags (single word)
  (s) =>
    /^(div|span|p|a|h[1-6]|ul|ol|li|table|thead|tbody|tfoot|tr|td|th|input|button|select|option|textarea|label|form|img|canvas|svg|path|circle|rect|line|text|g|defs|strong|em|b|i|small|br|hr|pre|code|section|article|header|footer|main|nav|aside|details|summary|figure|figcaption)$/.test(
      s,
    ),

  // HTML/DOM attribute names and values
  (s) =>
    /^(type|class|id|name|value|href|src|alt|title|role|scope|for|action|method|target|rel|disabled|readonly|checked|selected|required|placeholder|maxlength|minlength|pattern|autocomplete|autofocus|tabindex|colspan|rowspan|aria-\w+|data-\w+|accept|multiple|min|max|step|width|height|style|download|loading|crossorigin|integrity|referrerpolicy)$/.test(
      s,
    ),

  // DOM event names
  (s) =>
    /^(click|change|input|submit|focus|blur|keydown|keyup|keypress|mousedown|mouseup|mousemove|mouseenter|mouseleave|touchstart|touchend|scroll|resize|load|error|abort|pointerdown|pointerup|contextmenu|wheel|dragstart|dragend|drag|drop|dragover|dragenter|dragleave|paste|copy|cut)$/.test(
      s,
    ),

  // Keyboard key names
  (s) =>
    /^(Enter|Escape|Tab|Backspace|Delete|ArrowUp|ArrowDown|ArrowLeft|ArrowRight|Home|End|PageUp|PageDown|Space|Shift|Control|Alt|Meta|CapsLock|F[1-9]|F1[0-2])$/.test(
      s,
    ),

  // JavaScript typeof values
  (s) =>
    /^(string|number|boolean|undefined|object|function|symbol|bigint)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(primary|secondary|tertiary|ghost|outline|link|destructive|big|giant|small|medium|large|xs|sm|md|lg|xl|xxl|compact|full|mini|tiny)$/.test(
      s,
    ),

  // CSS font/style values
  (s) =>
    /^(bold|italic|normal|underline|uppercase|lowercase|capitalize|nowrap|pre|pre-wrap|pre-line|serif|sans-serif|monospace|cursive|fantasy)$/.test(
      s,
    ),

  // Scroll / intersection / animation behavior values
  (s) =>
    /^(smooth|instant|nearest|contain|cover|fill|scale-down|ease|ease-in|ease-out|ease-in-out|linear)$/.test(
      s,
    ),

  // Web Audio API values
  (s) =>
    /^(sine|square|sawtooth|triangle|lowpass|highpass|bandpass)$/.test(
      s,
    ),
  ...
  (s) =>
    ...
    s.length < 25 &&
    /^(volume|weight|reps|duration|distance|pace|heartRate|heartrate|heart_rate|calories|strength|cardio|flexibility|custom|exercise|workout|date|timestamp|notes|protocol|origine|actions|asc|desc|normal|warning|danger|success|info|error|complete|high|low|pending|active|default|both|merge|single|compact|manual|auto|replace|container|icon|dropdown|tag|group|showAddButton|searchByName|exactMatch|showControls|dateRange|targetWeight|targetReps|targetDistance|targetDuration|targetPace|targetHeartRate|musclegroup|muscle_group|sound|showTimer|timerSound|showLog|currentWorkout|sortBy|sortOrder|number|string)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(is-|has-|mod-|nav-|view-|workspace-|cm-|markdown-|setting-|clickable|tree-item)/.test(
      s,
    ),
  ...
  (s) =>
    /^(drop_set|super_set|superset|pyramid|rest_pause|giant_set|cluster|myo_reps|myoreps|emom|amrap|tabata)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(round|butt|square|miter|bevel|evenodd|nonzero|fill|stroke|ellipse|circle|rect|line|path|polygon|polyline|radialGradient|linearGradient|stop|clipPath|mask|marker|pattern|defs|use|symbol|image|foreignObject|animate|animateTransform|set|desc|metadata|title|switch|a|tspan|textPath)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(pie|doughnut|bar|line|radar|polarArea|scatter|bubble|horizontal)$/.test(
      s,
    ),

  // Canvas 2D context values
  (s) =>
    /^(pointer|crosshair|move|grab|grabbing|text|wait|not-allowed|no-drop|col-resize|row-resize|n-resize|e-resize|s-resize|w-resize|ne-resize|nw-resize|se-resize|sw-resize|ew-resize|ns-resize)$/.test(
      s,
    ),

  // Muscle group / body part identifiers (used as data keys, not displayed)
  (s) =>
    /^(chest|back|shoulders|biceps|triceps|forearms|abs|core|quads|hamstrings|glutes|calves|traps|lats|rear_delts|front_delts|side_delts|obliques|lower_back|hip_flexors|adductors|abductors|neck|pectorals|rhomboids|rotator_cuff|serratus)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(week|month|year|day|hour|minute|second|quarter|semester)$/.test(
      s,
    ),

  // Search / algorithm mode identifiers
  (s) =>
    /^(semantic|fuzzy|exact|partial|prefix|suffix|contains|regex)$/.test(
      s,
    ),

  // Canvas export / layout identifiers
  (s) =>
    /^(file|arrow|warmup|cooldown|stretching|routine|introduction|overview|instructions|grouped|stable|sequential|parallel)$/.test(
      s,
    ),

  // Unit abbreviations (3 chars or less, no spaces)
  (s) =>
    /^(sec|min|hrs|bpm|rpm|cal|lbs|kgs?|reps?|sets?|km|mi)$/.test(s),

  // Pluralization pairs (entry/entries pattern)
  (s) =>
    /^(entry|entries|item|items|set|sets|rep|reps|round|rounds)$/.test(
      s,
    ),
  ...
  // Input type values
  (s) =>
    /^(checkbox|radio|text|password|email|tel|url|search|date|time|datetime-local|month|week|number|range|color|file|hidden|image|reset|submit)$/.test(
      s,
    ),
  ...
  (s) =>
    /^(color|fill|stroke|opacity|background|font|display|visibility|transform|transition|animation|cursor|overflow|z-index|position|margin|padding|border|outline|box-shadow|text-align|text-decoration|vertical-align|line-height|letter-spacing|word-spacing|white-space|word-break|word-wrap|text-overflow|text-transform|text-indent|text-shadow|list-style|content|float|clear|direction|unicode-bidi|writing-mode)\s*:/.test(
      s,
    ),

  // SVG body-part identifiers (used as group IDs or class names in SVG templates)
  (s) =>
    /^(hands|feet|body|lowerback|gastrocnemius|soleus|infraspinatus|supraspinatus|teres|rhomboid|trapezius|deltoid|pectoralis|latissimus|serratus|rectus|oblique|erector|gluteus|quadricep|hamstring|tibialis|fibularis|iliopsoas|adductor|sartorius|gracilis|tensor|piriformis)$/.test(
      s,
    ),

  // Font family strings
  (s) =>
    /^['"]?[\w\s-]+['"]?,\s*(-apple-system|sans-serif|serif|monospace|system-ui)/.test(
      s,
    ),

  // Trend direction / comparison result identifiers
  (s) =>
    /^(up|down|neutral|stable|improving|declining|flat|positive|negative)$/.test(
      s,
    ),

  // Chart/data index mode identifiers
  (s) =>
    /^(index|dataset|point|nearest|average|single|label|x|y|xy)$/.test(
      s,
    ),
...
function scanFile(filePath) {
  ...
  const hasI18nImport =
    /import\s+\{[^}]*\bt\b[^}]*\}\s+from\s+["']@app\/i18n["']/.test(
      content,
    );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const lineIsUserFacing = USER_FACING_CONTEXT.some((re) =>
      re.test(line),
    );
...
if (allResults.length === 0) {
  console.log(
    "  All clean! No hardcoded user-facing strings found.\n",
  );
  process.exit(0);
}
...
if (confirmed.length > 0) {
  console.log(
    `  CONFIRMED user-facing (setText/textContent/Notice/text:/etc.): ${confirmed.length}`,
  );
...
if (likely.length > 0) {
  console.log(
    `  LIKELY hardcoded (review manually): ${likely.length}`,
  );
  console.log("  " + "─".repeat(60));
  ...
  for (const fileName of Object.keys(byFile).sort()) {
    ...
    for (const r of items) {
      ...
      if (fixHints) {
        console.log(
          `         -> t("${suggestI18nKey(r.string, fileName)}")`,
        );
      }
    }
  }
...
if (verbose && allSkipped.length > 0) {
  ...
  for (const [name, items] of Object.entries(byFilter)) {
    ...
    for (const item of items.slice(0, 5)) {
      console.log(
        `    ${path.relative(ROOT, item.file)}:${item.line}  ${JSON.stringify(item.string)}`,
      );
    }
    if (items.length > 5)
...
function suggestI18nKey(str, filePath) {
  const feature =
    filePath.match(/features\/([^/]+)/)?.[1] || "common";
  const cleaned = str
    .replace(/\$\{[^}]+\}/g, "")
    .replace(/[^a-zA-Z\s]/g, "")
    .trim()
